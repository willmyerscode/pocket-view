class PocketView {
  ...
  buildStructure(){
    ...
    this.contentItems = this.el.querySelectorAll('.pocket-view-wrapper .content-item');
    this.accordionItems = this.el.querySelectorAll('.pocket-accordion .content-item');

    this.buttons = this.el.querySelectorAll('.pocket-view-wrapper .pocket-button');
    this.accordionButtons = this.el.querySelectorAll('.pocket-accordion .pocket-button');
  ...
  buildButton(item, accordionItem) {
    ...
    if (presetContent.includes("one")) {
      ...
      let buttonText = document.createElement(`${this.settings.buttonTitleTag}`);
  ...
  buildContentPanel(item, accordionItem) {
    ...
    description = description.replace(/<p data-rte-preserve-empty="true" style="white-space:pre-wrap;">|<\/p>/g, '\n');
    ...
    description = description
        .replace(/<a[^>]*href=["']([^#"']+)#h1["'][^>]*>(.*?)<\/a>/g, '<h1><a href="$1">$2</a></h1>')
        .replace(/<a[^>]*href=["']([^#"']+)#h2["'][^>]*>(.*?)<\/a>/g, '<h2><a href="$1">$2</a></h2>')
        .replace(/<a[^>]*href=["']([^#"']+)#h3["'][^>]*>(.*?)<\/a>/g, '<h3><a href="$1">$2</a></h3>')
        .replace(/<a[^>]*href=["']([^#"']+)#h4["'][^>]*>(.*?)<\/a>/g, '<h4><a href="$1">$2</a></h4>')
    ...
    if (currentText.trim()) {
      ...
      if (this.buttonEnabled == true) {
        ...
        this.buttonLink.classList.add('list-item-content__button', 'sqs-block-button-element', 'sqs-block-button-element--medium', 'sqs-button-element--primary');
  ...
  setInitialState(){
    ...
    if (presetStyle.includes("block")) {
      this.el.classList.add('block');
    }

    if (presetStyle.includes("minimal")) {
      this.el.classList.add('minimal');
    }

    if (presetStyle.includes("bubble")) {
      this.el.classList.add('bubble');
    }

    if (mobileLayout.includes("scroll")) {
      this.el.classList.add('mobile-scroll');
    }

    if (mobileLayout.includes("accordion")) {
  ...
  watchInteraction(){
    ...
    else if (interactionType.includes('auto')) {
      this.autoPlay();
    }
  }
  ...
  hoverListen(){
    ...
    this.buttons.forEach((button, index) => {
      button.addEventListener('mouseenter', () => {
        ...
        if (window.innerWidth <= 766 && this.el.classList.contains('mobile-scroll')) {
  ...
  autoPlay(){
    ...
    function setActive() {
        ...
        this.accordionButtons[currentIndex].classList.add('active');
        this.contentItems[currentIndex].classList.add('active');
        this.accordionItems[currentIndex].classList.add('active');
    ...
    const intervalId = setInterval(setActive, `${this.settings.autoTiming}`);
  }
  ...
  setHeights() {
    ...
    const contentItems = document.querySelectorAll('.pocket-view-wrapper .content-item');
    ...
    contentItems.forEach(item => {
        ...
        const fullHeight = item.getBoundingClientRect().height + marginTop + marginBottom;
